...
class ControlStreamHandler(
    ...
    suspend fun run(channel: ManagedChannel): Unit = coroutineScope {
        ...
        stream.collect { msg ->
            ...
            when {
                msg.hasCreateContainer() -> handleCreate(msg.correlationId, msg.createContainer)
                msg.hasStartContainer() -> handleStart(msg.correlationId, msg.startContainer)
                msg.hasStopContainer() -> handleStop(msg.correlationId, msg.stopContainer)
                msg.hasRestartContainer() -> handleRestart(msg.correlationId, msg.restartContainer)
    ...
    private suspend fun handleCreate(correlationId: String, cmd: CreateContainerCommand) {
    ...
    private suspend fun handleStart(correlationId: String, cmd: StartContainerCommand) {
        log.info("Starting container ${cmd.containerName}")
        runCatching { containerManager.startContainer(cmd.containerName) }
            .onFailure { log.error("Failed to start container ${cmd.containerName}", it) }
    }

    private suspend fun handleStop(correlationId: String, cmd: StopContainerCommand) {
        ...
        runCatching { containerManager.stopContainer(cmd.containerName, cmd.timeoutSeconds, cmd.stopCommand) }
            .onFailure { log.error("Failed to stop container ${cmd.containerName}", it) }
    }

    private suspend fun handleRestart(correlationId: String, cmd: RestartContainerCommand) {
        ...
        }.onFailure { log.error("Failed to restart container ${cmd.containerName}", it) }
    }
