...
private class FakeConsoleSession(private val outputFlow: Flow<ByteArray>) : ConsoleSession {
...
private class FakeLogFetcher(private val result: List<String>?, private val throwOnFetch: Boolean = false) : LogFetcher {
...
class ConsoleHandlerTest :
    FunSpec({
        ...
        fun handler(sessionFactory: ConsoleSession.Factory, logFetcher: LogFetcher) = ConsoleHandler(sessionFactory, logFetcher, Dispatchers.Unconfined)
        ...
        test("input writes data to session") {
            runBlocking {
                val session = FakeConsoleSession(flow { awaitCancellation() })
                val factory = ConsoleSession.Factory { session }
                val logFetcher = FakeLogFetcher(emptyList())
                val handler = handler(factory, logFetcher)
                val channel = Channel<AgentMessage>(Channel.UNLIMITED)
        ...
        test("input arriving while attach is in flight is buffered and drained in order") {
            runBlocking {
                val session = FakeConsoleSession(flow { awaitCancellation() })
                ...
                val handler = handler(factory, FakeLogFetcher(emptyList()))
                val channel = Channel<AgentMessage>(Channel.UNLIMITED)
                val outbound = AgentOutbound(channel, nodeId)
                ...
                // Let the attach register its slot and suspend inside the factory.
                yield()
                handler.handleConsoleInput(
                    consoleInput {
                        requestId = "req-1"
                        data = com.google.protobuf.ByteString.copyFromUtf8("early-1\n")
                    }
                )
                handler.handleConsoleInput(
                    consoleInput {
                        requestId = "req-1"
                        data = com.google.protobuf.ByteString.copyFromUtf8("early-2\n")
                    }
                )
                release.complete(Unit)
                attachJob.join()

                session.writtenData.map { it.toString(Charsets.UTF_8) } shouldContainExactly
                    listOf("early-1\n", "early-2\n")
            }
        }
