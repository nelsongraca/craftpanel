...
import io.craftpanel.proto.copyFileRequest
import io.craftpanel.proto.deleteFileRequest
import io.craftpanel.proto.downloadBackupCommand
import io.craftpanel.proto.downloadFileCommand
import io.craftpanel.proto.listFilesRequest
import io.craftpanel.proto.makeDirectoryRequest
import io.craftpanel.proto.moveFileRequest
import io.craftpanel.proto.readFileRequest
import io.craftpanel.proto.uploadFileCommand
...
class FileHandlerTest : FunSpec({
    ...
    fun newOutbound(): Pair<AgentOutbound, Channel<io.craftpanel.proto.AgentMessage>> {
        val channel = Channel<io.craftpanel.proto.AgentMessage>(Channel.UNLIMITED)
    ...
    test("handleListFiles reports NOT_FOUND when the requested path escapes the server root") {
        ...
        runBlocking {
            handler.handleListFiles(listFilesRequest { requestId = "r1"; serverId = "missing-server"; path = "/../../etc" }, out)
        }
    ...
    test("handleReadFile reports NOT_FOUND when the file does not exist") {
        ...
        runBlocking {
            handler.handleReadFile(readFileRequest { requestId = "r1"; serverId = "srv-1"; path = "missing.txt" }, out)
        }
    ...
    test("handleWriteFile reports failure envelope on write error") {
        ...
        runBlocking {
            handler.handleWriteFile(writeFileRequest { requestId = "r1"; serverId = "srv-1"; path = "../escape.txt" }, out)
        }
    ...
    test("handleDeleteFile reports NOT_FOUND when the path does not exist") {
        ...
        runBlocking {
            handler.handleDeleteFile(deleteFileRequest { requestId = "r1"; serverId = "srv-1"; path = "missing.txt" }, out)
        }
    ...
    test("handleMakeDirectory reports failure envelope on path traversal") {
        ...
        runBlocking {
            handler.handleMakeDirectory(makeDirectoryRequest { requestId = "r1"; serverId = "srv-1"; path = "../escape" }, out)
        }
    ...
    test("handleMoveFile reports NOT_FOUND when the source does not exist") {
        ...
        runBlocking {
            handler.handleMoveFile(moveFileRequest { requestId = "r1"; serverId = "srv-1"; sourcePath = "missing.txt"; destinationPath = "dest.txt" }, out)
        }
    ...
    test("handleCopyFile reports NOT_FOUND when the source does not exist") {
        ...
        runBlocking {
            handler.handleCopyFile(copyFileRequest { requestId = "r1"; serverId = "srv-1"; sourcePath = "missing.txt"; destinationPath = "dest.txt" }, out)
        }
    ...
    test("handleDownloadFile reports NOT_FOUND when the file does not exist") {
        ...
        runBlocking {
            handler.handleDownloadFile(downloadFileCommand { requestId = "r1"; serverId = "srv-1"; path = "missing.txt"; transferId = "t1" }, bulkClient, out)
        }
    ...
    test("handleUploadFile reports failure envelope when the bulk transfer fails") {
        ...
        runBlocking {
            handler.handleUploadFile(uploadFileCommand { requestId = "r1"; serverId = "srv-1"; path = "dest.txt"; transferId = "t1" }, bulkClient, out)
        }
    ...
    test("handleDownloadBackup reports INTERNAL when the backup id is invalid") {
        ...
        runBlocking {
            handler.handleDownloadBackup(downloadBackupCommand { requestId = "r1"; backupId = "not-a-uuid"; transferId = "t1" }, bulkClient, out)
        }
