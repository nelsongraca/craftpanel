...
| Player Ingress    | `itzg/mc-router`               | One per node; named `craftpanel-mc-router-<node-id>` for Swarm uniqueness; provisioned by the agent on startup with `IN_DOCKER=true` and `DYNAMIC_PROXY_PROTOCOL=true`; routes by `mc-router.host` container labels |
...
Master and agents communicate over **gRPC with TLS**. The agent initiates a persistent bidirectional connection to master on startup, which means nodes behind NAT do not require inbound firewall
rules.
