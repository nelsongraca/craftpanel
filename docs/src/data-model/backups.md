...
!!! note
Retention is enforced by master before each new backup: if the server's `backup_max_count` limit is reached, the oldest `COMPLETED` backup is deleted first (agent removes the file, master removes the
row). `IN_PROGRESS` and `FAILED` records do not count toward the limit.
