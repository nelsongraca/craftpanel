...
!!! note
Exactly one of `threshold_value` or `threshold_state` should be non-null for a given row. Numeric thresholds fire when the metric exceeds the value (e.g. `ram_percent > 90`). State thresholds fire
when the metric equals the specified state string.
