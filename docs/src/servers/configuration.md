...
`server.properties` values are driven by **itzg environment variables**. The UI presents labelled form fields — users never interact with raw env var names. Master translates UI inputs to the correct
itzg env var format, stores them in the database, and injects them into the container spec on creation or restart.

This is the recommended mode for most users.

### Manual Mode

Disables env var management entirely. The panel provides direct file editor access to `server.properties` (and any other config file) via the [File Explorer](./files-console.md). The user is
responsible for file correctness.
...
!!! note
When a server is migrated to a different node, master automatically updates easy-mode proxy configs that reference it. Manual-mode proxy configs are not updated automatically — master will display a
warning.
