...
| `MEMORY` / `MAX_MEMORY`      | `memory_mb` column            | Heap = `memory_mb − (512 MB + 12.5%)`, floored so heap ≥ 50% (non-linear; see below). Overridable via a `MEMORY` env var |
| `SERVER_PORT`                | Port registry         | Assigned by master; user-configurable for `CUSTOM` servers via `container_listen_port`              |
...
| `ENABLE_QUERY`               | Not injected                  | Set it yourself only for external tooling that uses the UDP query protocol; player count does not use it |
| `QUERY_PORT`                 | Not injected                  | Set alongside `ENABLE_QUERY` if external tooling needs it             |
...
`MEMORY` is the JVM heap ceiling. Instead of a flat percentage, master reserves a fixed 512 MB for
JVM non-heap memory (Metaspace, thread stacks, code cache, entrypoint) plus 12.5% of the container
for heap-scaling overhead (GC structures, direct buffers), capped so the heap is never below 50% of
`memory_mb`:

| `memory_mb` | `MEMORY` (heap) | heap share |
|-------------|-----------------|------------|
| 1024        | 512M            | 50%        |
| 2048        | 1280M           | 62.5%      |
| 4096        | 3072M           | 75%        |
...
With Aikar's flags (`-Xms=-Xmx=MEMORY -XX:+AlwaysPreTouch`) the entire heap is committed at boot, so
an over-large heap leaves too little for the rest of the JVM and the container is OOMKilled before
it can log. A user-set `MEMORY` env var overrides this default.

### CUSTOM servers

`CUSTOM` servers run an arbitrary server jar (`itzg TYPE=CUSTOM`). The managed-mode defaults above are **never generated** for them — there is no form field → env var translation, and `config_mode` is
always `MANUAL` (master injects `OVERRIDE_SERVER_PROPERTIES=false` so itzg leaves `server.properties` untouched). Only the jar URL, container port/protocol, extra env vars (`server_env_vars`), and the
stop command are user-configurable.

| Variable              | Source                  | Notes                                                                                     |
|-----------------------|-------------------------|-------------------------------------------------------------------------------------------|
| `TYPE`                | `server_type` column    | Always `CUSTOM`                                                                           |
| `VERSION`             | Forced `LATEST`         | `mc_version` is ignored for CUSTOM servers                                                |
| `CUSTOM_SERVER`       | `custom_server_jar`     | Downloaded and run as the server jar                                                      |
| `DISABLE_HEALTHCHECK` | `disable_healthcheck`   | Set to `true` when the checkbox is enabled; omits itzg's health check (e.g. non-MC jars)   |
| `FORCE_REDOWNLOAD`    | `force_redownload`      | Set to `true` when the checkbox is enabled; re-downloads the jar on every start            |

`container_listen_port` becomes `SERVER_PORT` (falling back to `25565` when unset), and `container_protocol = UDP` exposes the host port mapping as UDP and skips mc-router labels (TCP-only).
...
| Variable(s)                                                                                                                                                          | Reason                                                                               |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
| `TYPE`, `VERSION`, `EULA`, `MEMORY`, `SERVER_PORT`, `RCON_*`, `ENABLE_QUERY`, `QUERY_PORT`, `ENABLE_ROLLING_LOGS`, `MODRINTH_PROJECTS`, `OVERRIDE_SERVER_PROPERTIES`, `CUSTOM_SERVER`, `DISABLE_HEALTHCHECK`, `FORCE_REDOWNLOAD` | Master-owned; see above                                                              |
...
`CUSTOM` servers are the exception: `mc_version` is always stored and injected as `LATEST` — the actual software version is determined by whatever `custom_server_jar` points to.
