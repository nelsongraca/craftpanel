...
class PrepareRsyncReceiveStepTest : FunSpec({
    ...
    fun ctx(scope: TestScope = TestScope()) = MigrationContext(
        ...
        scope = scope,
        eventFlow = null,
    ).apply { rsyncPort = 25566 }

    test("Success when gateway returns RsyncReadyEvent within timeout") {
        runTest {
            val c = ctx(this)
            launch {
                delay(50.milliseconds)
                events.emit(AgentEvent.RsyncReadyEvent("mig-1", "secret123"))
            }
            val result = PrepareRsyncReceiveStep().execute(c)
            result.shouldBeInstanceOf<StepResult.Success>()
            c.rsyncPassword shouldBe "secret123"
        }
    }
    ...
    test("Failure when sendToNode returns false") {
        runTest {
            ...
            val c = ctx(this).copy(gateway = badGateway)
            val result = PrepareRsyncReceiveStep().execute(c)
            result.shouldBeInstanceOf<StepResult.Failure>()
        }
    }

    test("Failure when no event within timeout") {
        runTest {
            val c = ctx(this)
            val result = PrepareRsyncReceiveStep().execute(c)
            result.shouldBeInstanceOf<StepResult.Failure>()
