...
class FinalRsyncStepTest : FunSpec({
    ...
    test("Success when final rsync completes") {
        runTest {
            val c = ctx(this)
            launch {
                delay(50.milliseconds)
                events.emit(AgentEvent.RsyncCompleteEvent("mig-1", true, true, ""))
            }
            val result = FinalRsyncStep().execute(c)
    ...
    test("Failure when complete.success is false") {
        runTest {
            ...
            launch {
                delay(50.milliseconds)
                events.emit(AgentEvent.RsyncCompleteEvent("mig-1", true, false, "rsync error"))
            }
            val result = FinalRsyncStep().execute(c)
