...
class StopSourceStepTest : FunSpec({
    ...
    test("Success when server already stopped") {
        runTest {
            val result = StopSourceStep().execute(ctx)
            result.shouldBeInstanceOf<StepResult.Success>()
            ctx.sourceStopped shouldBe true
        }
    }

    test("Failure when server running - lifecycle stop returns failure") {
        runTest {
            val runningRow = ctx.serverRow.copy(status = "RUNNING")
            val runningCtx = ctx.copy(serverRow = runningRow)
            val result = StopSourceStep().execute(runningCtx)
