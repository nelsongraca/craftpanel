...
describe("WsProvider", () => {
    ...
    it("replays the last snapshot to a late subscriber", () => {
        ...
        const snapshot = {servers: [{id: "s1", metrics: {ram_used_mb: 512}}]};
        capturedOnEvent.current!("snapshot", snapshot);

        const listener = vi.fn();
        capturedSubscribe!("snapshot", listener);

        expect(listener).toHaveBeenCalledWith(snapshot);
    });

    it("does not replay a snapshot that was never received", () => {
        let capturedSubscribe: ((type: string, listener: AnyFn) => () => void) | null = null;
        function Consumer() {
            const {subscribe} = useWs();
            capturedSubscribe = subscribe;
