...
describe("ProxySettingsSection", () => {
    ...
    it("renders settings after loading", async () => {
        vi.mocked(getProxySettings).mockResolvedValue({
            data: {motd: "A Proxy", max_players: 20, forwarding_mode: "MODERN", proxy_protocol: false},
            error: undefined,
    ...
    it("shows forwarding mode select for VELOCITY", async () => {
        vi.mocked(getProxySettings).mockResolvedValue({
            data: {motd: "", max_players: null, forwarding_mode: "", proxy_protocol: false},
            error: undefined,
            response: new Response(),
        });
        render(<ProxySettingsSection serverId="s1" serverType="VELOCITY" />);
    ...
    it("calls updateProxySettings on save", async () => {
        vi.mocked(getProxySettings).mockResolvedValue({
            data: {motd: "Hello", max_players: 20, forwarding_mode: "NONE", proxy_protocol: false},
            error: undefined,
            response: new Response(),
        });
        vi.mocked(updateProxySettings).mockResolvedValue({
            data: {forwarding_warnings: []},
            error: undefined,
            response: new Response(),
        });
        ...
        expect(updateProxySettings).toHaveBeenCalledWith({
            path: {id: "s1"},
            body: {motd: "New MOTD", max_players: 20, forwarding_mode: "NONE", proxy_protocol: false},
        });
    });

    it("toggles PROXY protocol and sends it on save", async () => {
        vi.mocked(getProxySettings).mockResolvedValue({
            data: {motd: "", max_players: null, forwarding_mode: "", proxy_protocol: false},
            error: undefined,
            response: new Response(),
        ...
        const toggle = await screen.findByRole("checkbox", {name: /PROXY Protocol/i});
        ...
        expect(updateProxySettings).toHaveBeenCalledWith({
            path: {id: "s1"},
            body: {motd: null, max_players: null, forwarding_mode: null, proxy_protocol: true},
        });
    });

    it("reflects a persisted PROXY protocol setting on load", async () => {
        vi.mocked(getProxySettings).mockResolvedValue({
            data: {motd: "", max_players: null, forwarding_mode: "", proxy_protocol: true},
            error: undefined,
            response: new Response(),
        });
        render(<ProxySettingsSection serverId="s1" serverType="VELOCITY" />);
        expect(await screen.findByRole("checkbox", {name: /PROXY Protocol/i})).toBeChecked();
    });
