...
import {
  ...
  listGroups, listUserAssignments, createAssignment, deleteAssignment,
  listNetworks, listServers,
} from "@/lib/generated/sdk.gen";
import type { User, Group, Assignment } from "@/lib/types";
...
function CreateUserModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(false);
    if (res.error) { setError((res.error as { message?: string }).message ?? "Failed"); return; }
    onDone();
  }
...
function AssignmentsModal({
  ...
  useEffect(() => {
    loadAssignments();
    listServers().then((r) => { if (r.data) setServers(r.data); });
    listNetworks().then((r) => { if (r.data) setNetworks(r.data); });
  }, [loadAssignments]);

  async function handleAdd() {
    setAddError("");
    if (!newGroup) { setAddError("Select a group"); return; }
    if (newScope !== "GLOBAL" && !newScopeId) { setAddError("Select a scope target"); return; }
    const res = await createAssignment({
      path: { userId: user.id },
      body: { group_id: newGroup, scope_type: newScope, scope_id: newScope !== "GLOBAL" ? newScopeId : undefined },
    });
    if (res.error) { setAddError((res.error as { message?: string }).message ?? "Failed"); return; }
    setNewGroup("");
    setNewScope("GLOBAL");
    setNewScopeId("");
    loadAssignments();
    ...
    <Modal title={`Groups — ${user.username}`} onClose={onClose}>
      <div className="space-y-5">
        ...
        <div className="border-t border-border pt-4 space-y-3">
          ...
          <Field label="Scope">
            <select className={INPUT} value={newScope} onChange={(e) => { setNewScope(e.target.value); setNewScopeId(""); }}>
...
export default function UsersPage() {
  ...
  async function handleDelete() {
    ...
    if (res.error) { setDeleteError((res.error as { message?: string }).message ?? "Failed"); return; }
    setDeleting(null);
    load();
  }
  ...
  return (
    <div>
                ...
                {users.map((u) => (
                  <tr key={u.id} className="border-b border-border/50 hover:bg-surface-high/40">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-heading font-bold uppercase tracking-wider border ${u.is_active ? "text-healthy border-healthy/30 bg-healthy/10" : "text-text-muted border-border bg-surface-high"}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        ...
                        <button
                          onClick={() => { setDeleting(u); setDeleteError(""); }}
      ...
      {showCreate && (
        <CreateUserModal onClose={() => setShowCreate(false)} onDone={() => { setShowCreate(false); load(); }} />
      )}

      {editing && (
        <EditUserModal user={editing} onClose={() => setEditing(null)} onDone={() => { setEditing(null); load(); }} />
      )}
