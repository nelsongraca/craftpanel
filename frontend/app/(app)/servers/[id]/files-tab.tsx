...
import { ChevronRight, ChevronDown, Folder, File, Upload, FolderPlus, Download, Trash2, Pencil, Save, X } from "lucide-react";
...
export function FilesTab({ serverId }: Props) {
  ...
  async function openFile(node: TreeNode) {
    if (node.isDirectory) { void toggleDir(node); return; }
    ...
    if (err || !data) { setError("Failed to load file"); return; }
  ...
  async function saveFile() {
    ...
    setSavingFile(false);
    if (!res.ok) { setError("Failed to save file"); return; }
    setDirty(false);
  }
  ...
  async function deleteEntry(path: string, isDir: boolean) {
    ...
    if (err) { setError("Failed to delete"); return; }
    if (selectedPath === path) { setSelectedPath(null); setFileContent(""); }
  ...
  async function mkdirPrompt() {
    ...
    if (err) { setError("Failed to create directory"); return; }
    setRoots(await loadDir("/"));
  }
  ...
  function renderTree(nodes: TreeNode[], depth = 0): React.ReactNode {
    return nodes.map((node) => (
      <div key={node.path}>
        <div
          className={[
            "flex items-center gap-1.5 px-2 py-0.5 cursor-pointer text-[12px] rounded select-none group",
          ...
          onClick={() => { if (node.isDirectory) void toggleDir(node); else void openFile(node); }}
        >
          {node.isDirectory ? (
            node.loading
          ...
          {renameNode?.path === node.path ? (
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Enter") void commitRename(); if (e.key === "Escape") setRenameNode(null); }}
          ...
          <span className="hidden group-hover:flex items-center gap-0.5 shrink-0">
            <button
              ...
              onClick={(e) => { e.stopPropagation(); void startRename(node); }}
            >
              <Pencil size={10} />
            </button>
            {!node.isDirectory && (
            ...
            <button
              ...
              onClick={(e) => { e.stopPropagation(); void deleteEntry(node.path, node.isDirectory); }}
            >
              <Trash2 size={10} />
            </button>
          </span>
        ...
        {selectedPath ? (
          <>
            ...
            <div className="flex-1 overflow-auto">
              {loadingFile ? (
                <p className="text-text-muted text-[12px] p-4">Loading…</p>
              ) : fileEncoding === "binary" ? (
                <p className="text-text-muted text-[12px] p-4">Binary file — use the download button to retrieve it.</p>
              ) : (
                <textarea
                  className="w-full h-full bg-bg font-mono text-[12px] text-text-primary p-4 resize-none focus:outline-none leading-relaxed"
                  value={fileContent}
          ...
          <div className="flex-1 flex items-center justify-center">
            <p className="text-text-muted text-[12px]">Select a file to edit</p>
          </div>
        )}
      </div>
    </div>
