...
describe("Config Mode Toggle", () => {
    ...
    it("confirming switch calls updateConfigMode with MANUAL and shows Manual mode description", async () => {
        ...
        await waitFor(() => expect(screen.getByText(/edit server\.properties directly/i)).toBeInTheDocument());
    });
    ...
    it("in MANUAL mode, mapped sections are hidden but JVM Options stays visible; Extra Vars still editable", async () => {
        await renderGameServer({configMode: "MANUAL"});

        expect(screen.queryByText("Gameplay")).not.toBeInTheDocument();
        expect(screen.queryByText("World")).not.toBeInTheDocument();
        // JVM Options has no server.properties equivalent, so it stays editable in manual mode
        expect(screen.getByText("JVM Options")).toBeInTheDocument();
    ...
    it("entering custom properties → Save → CUSTOM_SERVER_PROPERTIES key persisted", async () => {
        ...
        await waitFor(() =>
            expect(replaceEnvVars).toHaveBeenCalledWith(
                expect.objectContaining({
                    body: expect.objectContaining({
                        env_vars: expect.arrayContaining([{key: "CUSTOM_SERVER_PROPERTIES", value: "some.prop=value"}]),
                    }),
                }),
            ),
        );
...
describe("JVM Options Section", () => {
    ...
    it("JVM Options section stays visible after switching to MANUAL mode", async () => {
        const user = userEvent.setup();
        await renderGameServer({configMode: "MANAGED"});
        ...
        await waitFor(() => expect(screen.getByText("JVM Options")).toBeInTheDocument());
    });
...
describe("Proxy Server Config Tab", () => {
    ...
    it("removing backend → Save → backend absent in replaceProxyBackends", async () => {
        const user = userEvent.setup();
        const backends = [{id: "b1", backend_server_id: "s1", backend_name: "alpha", order: 1}];
        const servers: Partial<ServerResponse>[] = [
            {id: "s1", display_name: "Alpha", server_type: "PAPER", status: "HEALTHY", network_id: "n1"},
        ];
        await renderProxyServer(backends, servers as ServerResponse[]);
        ...
        await waitFor(() =>
            expect(replaceProxyBackends).toHaveBeenCalledWith(expect.objectContaining({body: {backends: []}})),
        );
    });
    ...
    it("duplicate backend names → Save blocked with error", async () => {
        ...
        await waitFor(() => expect(screen.getByText("Backend names must be unique")).toBeInTheDocument());
    ...
    it("updateProxySettings warn-skip surfaces in Proxy Settings section", async () => {
        ...
        vi.mocked(updateProxySettings).mockResolvedValue({
            data: {
                ...
                forwarding_warnings: [
                    "backend-1 is in MANUAL config mode — forwarding config not applied; configure it by hand",
                ],
            },
        } as never);
