...
import { MoreHorizontal, X, Check, Ban, KeyRound, Power, Trash2, Pencil } from "lucide-react";
...
function EditModal({
  ...
  async function save() {
    ...
    try {
      ...
      if (e) { setError(e.message ?? "Failed to save"); } else { onSaved(); onClose(); }
    } catch {
...
export default function NodesPage() {
  ...
  async function doTrust(nodeId: string) {
    ...
    try {
      ...
      else { await fetchNodes(); }
    } catch { setActionError("Failed to trust node"); }
    finally { setPendingAction((p) => { const n = { ...p }; delete n[nodeId]; return n; }); }
  }
  ...
  async function doReject(nodeId: string) {
    ...
    } catch { setActionError("Failed to reject node"); }
    finally { setPendingAction((p) => { const n = { ...p }; delete n[nodeId]; return n; }); }
  }
  ...
  async function doRotateToken(nodeId: string) {
    ...
    try {
      ...
      if (error) { setActionError(error.message ?? "Failed to rotate token"); }
      else if (data) { setTokenKey(data.node_key); }
    } catch { setActionError("Failed to rotate token"); }
    finally { setPendingAction((p) => { const n = { ...p }; delete n[nodeId]; return n; }); }
  }
  ...
  async function doShutdown(nodeId: string, displayName: string) {
    ...
    } catch { setActionError("Failed to shutdown node"); }
    finally { setPendingAction((p) => { const n = { ...p }; delete n[nodeId]; return n; }); }
  }
  ...
  async function doDecommission(node: Node) {
    ...
    } catch { setActionError("Failed to decommission node"); }
    finally { setPendingAction((p) => { const n = { ...p }; delete n[node.id]; return n; }); }
  }
                          ...
                          <>
                            ...
                            <div className="relative">
                              ...
                              {openMenuId === node.id && (
                                ...
                                >
                                  <button
                                    onClick={() => { setOpenMenuId(null); setEditNode(node); }}
                                  ...
                                  <button
                                    onClick={() => { setOpenMenuId(null); doRotateToken(node.id); }}
                                    className="flex items-center gap-2 w-full text-left px-3 py-1.5 text-[12px] font-heading font-bold uppercase tracking-wider text-text-primary hover:bg-surface-high transition-colors"
                                  >
                                  ...
                                  {ns === "ACTIVE" && (
                                    <button
                                      onClick={() => { setOpenMenuId(null); doShutdown(node.id, node.display_name); }}
                                  ...
                                  {servers === 0 && (
                                    <button
                                      onClick={() => { setOpenMenuId(null); doDecommission(node); }}
